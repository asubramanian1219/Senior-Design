...
#define samplingRate 5 // The time in between posts, in seconds
...
//  while (!fona.getGPS(&latitude, &longitude, &speed_kph, &heading, &altitude, &year, &month, &day, &hour, &minute, &second)) {
  ...
  while (!fona.getGPS(&latitude, &longitude, &speed_kph, &heading, &altitude)) {
    ...
    sprintf(URL, "http://arduinotestecen403.000webhostapp.com/firebaseTest.php?arduino_data=999,999");
  ...
  */
  ...
  counter = 0; // This counts the number of failed attempts tries
    ...
    sprintf(URL, "http://arduinotestecen403.000webhostapp.com/firebaseTest.php?arduino_data=%s,%s",latBuff, longBuff);
